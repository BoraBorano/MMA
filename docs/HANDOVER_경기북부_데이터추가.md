...
> **✅ 이 인계는 2026-08-02에 완료되었다 (PR #4 머지).**
> 아래 "현재 상태"·"권장 작업 순서"는 **당시 시점의 기록**이며 지금은 유효하지 않다.
> 실행 결과와 계획 대비 차이는 [GYEONGGI_NORTH_DATA_PLAN.md](./GYEONGGI_NORTH_DATA_PLAN.md) §9,
> 남은 과제는 [QA_BACKLOG.md](./QA_BACKLOG.md)를 보라.
> 이 문서는 §7(설계 원칙)과 §8(환경 함정)이 계속 유효하므로 보존한다.
...
- **작업 시작 전 `npm run convert:data`나 `npm test`가 실제로 도는지 먼저 확인할 것.** 2026-08-02 세션에서 `node_modules`의 `esbuild` 패키지가 누락되어(`.bin` 심을 남아 있는데 패키지가 없는 상태) `tsx`·`vite`·`vitest`가 전부 실행 불가였다. `npm install`로 복구된다. 이 상태에서는 변환·테스트·빌드 어느 것도 안 되므로 원인을 데이터나 코드에서 찾다 시간을 버리기 쉽다
- 엑셀을 열어둔 채 작업하면 `data-source/~$*.xlsx` 잠금 파일이 생긴다. `.gitignore`에 `~$*`를 추가해뒀으나 **아직 커밋 안 됨** *(→ `230369d`로 커밋 완료)*
